...
async def load_and_start(dut, target, current, max_attempts):
    await sample(dut, pack(CMD_LOAD_TARGET, target))
    await sample(dut, pack(CMD_LOAD_CURRENT, current))
    return await sample(dut, pack(CMD_CONTROL, (max_attempts << 2) | 1))
...
async def test_up_verify_down_verify_timeout_and_clear(dut):
    ...
    await reset(dut)
    uo, uio = await load_and_start(dut, target=8, current=0, max_attempts=15)
    assert uo == 0x41
    assert (uio & 0x0F) == 0x01
    uo, uio = await sample(dut, pack(CMD_CONTROL, 0x02))
    assert uo == 0x81
    ...
    await reset(dut)
    uo, uio = await run_case(dut, target=7, current=7, max_attempts=1)
    assert uo == 0xA3
    assert uio == 0x02

    await reset(dut)
    uo, uio = await run_case(dut, target=3, current=0, max_attempts=0)
    assert uo == 0xB0
    assert uio == 0x13
